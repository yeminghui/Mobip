...
from lib.utils import smp_metrics
...
def validate(epoch,config, val_loader, val_dataset, model, criterion, output_dir,
    ...
    num_class = 3
    miou_ls = []
    iou_ls = [[] for _ in range(num_class)]
    acc_ls = [[] for _ in range(num_class)]
    ...
    for batch_i, (img, target, paths, shapes) in tqdm(enumerate(val_loader), total=len(val_loader)):
        ...
        with torch.no_grad():
            ...
            # Segment evaluation(MULTICLASS_MODE)
            segmentation = seg_out.log_softmax(dim=1).exp()
            _, segmentation = torch.max(segmentation, 1)  # (bs, C, H, W) -> (bs, H, W)


            seg_predict = segmentation[:, pad_h:height - pad_h, pad_w:width - pad_w]
            seg_gt = target[1]
            seg_gt = seg_gt[:, pad_h:height - pad_h, pad_w:width - pad_w]
            tp_seg, fp_seg, fn_seg, tn_seg = smp_metrics.get_stats(seg_predict, seg_gt, mode='multiclass',
                                                                   threshold= None,
                                                                   num_classes=3 )
            ...
            for i in range(num_class):
                # batchsize是5， list中第一个list装的是5个iou， 第二个list同理
                iou_ls[i].append(iou.T[i].detach().cpu().numpy())
                acc_ls[i].append(acc.T[i].detach().cpu().numpy())
            
            total_loss, head_losses = criterion((train_out, seg_out), target, shapes,model)   #Compute loss
            ...
            # '''
            if config.TEST.PLOTS and batch_i == 0:
                for i in range(test_batch_size):
                    img_test = cv2.imread(paths[i])

                    seg_pred_i = seg_out[i][:, pad_h:height-pad_h, pad_w:width-pad_w].unsqueeze(0)
                    seg_pred_i = torch.nn.functional.interpolate(seg_pred_i, scale_factor=int(1 / ratio),
                                                                  mode='bilinear')
                    seg_pred_i = seg_pred_i.log_softmax(dim=1).exp()
                    _, seg_pred_i = torch.max(seg_pred_i, 1)
                    da_seg_mask = seg_pred_i.clone()
                    da_seg_mask[da_seg_mask==2] = 0
                    ll_seg_mask = seg_pred_i.clone()
                    ll_seg_mask[ll_seg_mask == 1] = 0
                    ll_seg_mask[ll_seg_mask == 2] = 1

                    seg_gt_i = target[1][i][ pad_h:height-pad_h, pad_w:width-pad_w].unsqueeze(0)
                    da_gt_mask = seg_gt_i.clone()
                    da_gt_mask[da_gt_mask==2] = 0
                    da_gt_mask = torch.cat([1-da_gt_mask,da_gt_mask],dim=0).float().unsqueeze(0)
                    ll_gt_mask = seg_gt_i.clone()
                    ll_gt_mask[ll_gt_mask == 1] = 0
                    ll_gt_mask[ll_gt_mask == 2] = 1
                    ll_gt_mask = torch.cat([1 - ll_gt_mask, ll_gt_mask], dim=0).float().unsqueeze(0)
    ...
    for i in range(num_class):  # 对3个类分别求iou的均值，
        iou_ls[i] = np.mean(iou_ls[i])
        acc_ls[i] = np.mean(acc_ls[i])

    da_acc_seg.update(acc_ls[1], 1)
    da_IoU_seg.update(iou_ls[1], 1)
    da_mIoU_seg.update(miou_ls[0], 1)

    ll_acc_seg.update(acc_ls[2], 1)
    ll_IoU_seg.update(iou_ls[2], 1)
    ll_mIoU_seg.update(miou_ls[1], 1)
