...
from lib.utils import add_scalar_list
...
def main():
    ...
    if args.local_rank != -1:
        ...
        dist.init_process_group(backend='nccl', init_method='env://', timeout=datetime.timedelta(seconds=54000))  # distributed backend
    ...
    Seg_Head_para_idx = [str(i) for i in range(25, 32)]
    # Seg_Head_para_idx = []
    ...
    # Freeze parameters in some part of the Model
    ...
    if cfg.TRAIN.DET_ONLY:  #Only train detection branch
        # logger.info('freeze encoder and Seg heads...')
        # print(model.named_parameters)
        for k, v in model.named_parameters():
            v.requires_grad = True  # train all layers
            if k.split(".")[1] in Encoder_para_idx + Seg_Head_para_idx:
                print('freezing %s' % k)
                v.requires_grad = False
    ...
    if cfg.TRAIN.ENC_DET_ONLY or cfg.TRAIN.DET_ONLY:    # Only train encoder and detection branchs
        # logger.info('freeze Seg heads...')
        for k, v in model.named_parameters():
            v.requires_grad = True  # train all layers
            if k.split(".")[1] in Seg_Head_para_idx:
                print('freezing %s' % k)
                v.requires_grad = False
    ...
    train_loader = DataLoaderX(
        ...
        collate_fn=dataset.BddDataset.collate_fn
    )
    ...
    for epoch in range(begin_epoch+1, cfg.TRAIN.END_EPOCH+1):
        ...
        if (epoch % cfg.TRAIN.VAL_FREQ == 0 or epoch == cfg.TRAIN.END_EPOCH) and rank in [-1, 0]:
            ...
            logger.info(msg)
            name_list=['Det_Recall','Det_mAP50','da_seg_mIOU','LL_Acc','LL_IOU']
            value_list=[detect_results[1],detect_results[2],da_segment_results[2],
                        ll_segment_results[0],ll_segment_results[1]]
            add_scalar_list(writer_dict['writer'],name_list,value_list,epoch)
